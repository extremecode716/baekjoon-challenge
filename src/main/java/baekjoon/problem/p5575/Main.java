...
public class Main {
    ...
    private static void solution() {
        List<String> inputs = Algorithm.readLines(3);

        StringBuilder stringBuilder = new StringBuilder();
        for (String input : inputs) {
            String[] split = input.split("\\s+");
            int second = (Integer.parseInt(split[3]) * 3600 + Integer.parseInt(split[4]) * 60 + Integer.parseInt(split[5])) - (Integer.parseInt(split[0]) * 3600 + Integer.parseInt(split[1]) * 60 + Integer.parseInt(split[2]));

            stringBuilder.append(second / 3600 % 24).append(" ").append(second / 60 % 60).append(" ").append(second % 60).append("\n");
        }
